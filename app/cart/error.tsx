...
export default function CartError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-[50vh] flex-col items-center justify-center px-4">
      <h1 className="mb-2 text-xl font-semibold">Could not load your cart</h1>
      ...
      <div className="flex gap-3">
        ...
        <Link href="/products" className="rounded-md bg-muted px-4 py-2 text-sm text-foreground hover:bg-muted/80">Continue shopping</Link>
      </div>
    </div>
  )
}
