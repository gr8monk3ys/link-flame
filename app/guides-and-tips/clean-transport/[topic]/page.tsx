...
export function generateMetadata({
  ...
  return params.then(({ topic: rawTopic }) => {
    ...
    return {
      title: `${topic} | Clean Transportation`,
      description: `Sustainable transportation guidance on ${topic.toLowerCase()}.`,
    };
  });
}

export default async function CleanTransportTopicPage({
  params,
}: {
  params: Promise<{ topic: string }>;
}) {
      ...
      <div className="mb-8">
        <nav aria-label="Breadcrumb" className="text-sm text-muted-foreground">
          <ol className="flex flex-wrap items-center gap-1">
            ...
            <li><Link href="/guides-and-tips/clean-transport" className="hover:underline">Clean Transportation</Link></li>
          </ol>
        </nav>
